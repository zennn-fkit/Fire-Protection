...
function EnvMetric({ icon: Icon, label, value, unit, threshKey, color }) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(10,22,40,0.6)', borderRadius: 12, border: `1px solid #1a3558`,
      flex: 1,
    }}>
      <div style={{
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginTop: 2 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 24, fontWeight: 800, color: '#e2e8f0' }}>
            {typeof value === 'number' ? value.toFixed(1) : value}
          </span>
          <span style={{ fontSize: 13, color: '#94a3b8', fontWeight: 600 }}>{unit}</span>
        </div>
        <div style={{ fontSize: 10, color: sc.text, fontWeight: 600, marginTop: 2 }}>{sc.label}</div>
      </div>
    </div>
  );
}
